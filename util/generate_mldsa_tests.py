...
class Key:
    ...
    def __init__(self, k_l: int, seed: bytes) -> None:
        self.k_l = k_l
        self.seed = seed
        self.public, self.secret = PURE[k_l]._keygen_internal(seed)

    def sign_message(self, message: bytes, deterministic: bool) -> bytes:
        return PURE[self.k_l].sign(self.secret, message,
                                   deterministic=deterministic)

# Key pairs to test with.
KEYS = {k_l: [Key(k_l, seed) for seed in SEEDS]
        for k_l in sorted(PURE.keys())}
...
class API:
    ...
    @classmethod
    def function(cls, func: str, k_l: int) -> str:
...
def one_mldsa_key_pair_from_seed(key: Key,
    ...
    tc.set_function(f'key_pair_from_seed_{key.k_l}')
    tc.set_dependencies([f'TF_PSA_CRYPTO_PQCP_MLDSA_{key.k_l}_ENABLED'])
    tc.set_arguments([
        test_case.hex_string(key.seed),
        test_case.hex_string(key.secret),
        test_case.hex_string(key.public),
    ])
    tc.set_description(f'MLDSA-{key.k_l} key pair from seed {descr}')
    return tc

def gen_pqcp_key_management(k_l: int) -> Iterable[test_case.TestCase]:
    """Generate test cases for mldsa-native keypair_internal()."""
    for i, key in enumerate(KEYS[k_l], 1):
...
def one_mldsa_sign_deterministic_pure(api: API,
    ...
    tc = test_case.TestCase()
    tc.set_function(api.function('sign_deterministic_pure', key.k_l))
    tc.set_dependencies([f'TF_PSA_CRYPTO_PQCP_MLDSA_{key.k_l}_ENABLED'])
    tc.set_arguments(api.metadata_arguments(key.k_l, True, True) + [
    ...
    tc.set_description(f'MLDSA-{key.k_l} sign deterministic {descr}')
    return tc
...
def one_mldsa_verify_pure(api: API,
    ...
    signature = key.sign_message(message, deterministic=True)
    tc = test_case.TestCase()
    tc.set_function(api.function('verify_pure', key.k_l))
    tc.set_dependencies([f'TF_PSA_CRYPTO_PQCP_MLDSA_{key.k_l}_ENABLED'])
    tc.set_arguments(api.metadata_arguments(key.k_l, False, True) + [
        test_case.hex_string(key.public),
        test_case.hex_string(message),
        test_case.hex_string(signature),
    ] + api.final_arguments())
    variant = "deterministic" if deterministic else "randomized"
    tc.set_description(f'MLDSA-{key.k_l} verify {variant} {descr}')
    return tc

def gen_mldsa_pure(api: API, k_l: int) -> Iterable[test_case.TestCase]:
    ...
    for message, descr in MESSAGES[1:]:
        yield one_mldsa_sign_deterministic_pure(api, KEYS[k_l][0], message,
                                                f'key#1 {descr}')
    for i, key in enumerate(KEYS[k_l], 1):
        yield one_mldsa_verify_pure(api, key, MESSAGES[0][0], True,
                                    f'key#{i}')
    for message, descr in MESSAGES[1:]:
        yield one_mldsa_verify_pure(api, KEYS[k_l][0], message,
                                    True, f'key#1 {descr}')
    for i, key in enumerate(KEYS[k_l], 1):
...
def gen_pqcp_mldsa_all() -> Iterable[test_case.TestCase]:
    ...
    api = PQCPAPI()
    for k_l in sorted(KEYS.keys()):
        yield from gen_pqcp_key_management(k_l)
        yield from gen_mldsa_pure(api, k_l)
