...
pre_check_environment () {
    ...
    source "$MBEDTLS_FRAMEWORK/scripts/project_detection.sh"
...
setup_quiet_wrappers()
{
    ...
    if [[ -e "${MBEDTLS_FRAMEWORK}/scripts/quiet" ]]; then
        export PATH="${MBEDTLS_FRAMEWORK}/scripts/quiet:$PATH"
    fi
}
...
cleanup()
{
    ...
    rm -f pkgconfig/Makefile "${MBEDTLS_FRAMEWORK}/Makefile"
