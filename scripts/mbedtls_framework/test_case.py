...
class TestSuite:
    ...
    The test suite object can be used with the `in` operator to test if a
    ...
    def _strip_test_prefix(name: str) -> str:
        """Strip the test_ prefix added by the function file parser."""
        assert name[:5] == 'test_'
        return name[5:]
    ...
    def _read_functions(self) -> Iterable[str]:
        ...
        # The keys in func_info are the test function names with
        # 'test_' prefixed to them.
        return [self._strip_test_prefix(key) for key in func_info.keys()]
